...
// struct for sending data through pipe to parent
typedef struct {
    char name[PIPE_NAME_LEN + 1];
    int count;
} NameCountData;

// remove newline/cr at end of string
...
// send our results back to parent through the pipe
static void write_results_to_pipe(int pipe_fd, NameCount counts[], int numDistinct) {
    ssize_t nw;
    nw = write(pipe_fd, &numDistinct, sizeof(int));  // tell parent how many entries
...
// each child writes to its own .out and .err files
...
int main(int argc, char *argv[]) {
    ...
    // shell passes pipe fd as last arg when spawning us
    if (argc >= 3) {
        pipe_fd = atoi(argv[argc - 1]);
        file_end = argc - 1;  // don't treat pipe fd as a filename
    }
    ...
    // write to pipe so parent can aggregate (send even if empty so it doesn't block)
    if (pipe_fd >= 0) {
